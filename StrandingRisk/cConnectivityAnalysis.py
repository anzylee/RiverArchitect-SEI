...
class ConnectivityAnalysis:
    ...
    def __init__(self, condition, species, lifestage, units, *args, **kwargs):
        ...
        try:
            ...
            self.out_dir = os.path.join(self.out_dir, f"flow_red_{fGl.write_Q_str(self.q_high)}_{fGl.write_Q_str(self.q_low)}")
        except:
    ...
    def get_target_raster(self):
        ...
        self.logger.info(f"Using Q = {Q_min} {self.q_units} for target")
        ...
        areas_shp_path = os.path.join(self.cache, f"areas{fGl.write_Q_str(Q_min)}.shp")
        ...
        self.logger.info('Identifying main channel...')
        all_areas = areas_shp_path
        # ensure areas exist (otherwise no target can be created)
        if arcpy.management.GetCount(all_areas)[0] == "0":
            raise Exception(f'ERROR: no target area at {Q_min}. Ensure depths exist above threshold {self.h_min} {self.length_units}.')

        # set output paths/environment
        arcpy.env.extent = Raster(self.Q_h_interp_dict[Q_min])  # target needs matching extent so matrices align
        target_shp = os.path.join(self.cache, "target.shp")
        self.target = os.path.join(self.out_dir, "target.tif")

        # determine largest continuous area
        max_area = max([value for (key, value) in arcpy.da.SearchCursor(all_areas, ['OID@', 'Area'])])
        exp = "Area >= %f" % max_area

        # save largest continuous area to target .shp
        arcpy.Select_analysis(all_areas, target_shp, exp)
        # convert target .shp to raster
        cell_size = arcpy.GetRasterProperties_management(self.Q_h_interp_dict[Q_min], 'CELLSIZEX').getOutput(0)
        arcpy.FeatureToRaster_conversion(target_shp, 'gridcode', self.target, cell_size)
        # delete intermediate products (no longer needed, also removes schema lock issue)
        arcpy.Delete_management(target_shp)
        self.logger.info("OK.")
    ...
    def make_shortest_paths_map(self, Q):
        ...
        self.logger.info(f"Discharge: {Q} {self.q_units}")
        self.logger.info("Physical Habitat: %s - %s" % (self.species, self.lifestage))
        self.logger.info("\tminimum swimming depth  = %s %s" % (self.h_min, self.length_units))
        self.logger.info("\tmaximum swimming speed  = %s %s" % (self.u_max, self.u_units))
        ...
        out_ras_name = os.path.join(self.shortest_paths_dir, f"path_lengths{fGl.write_Q_str(Q)}.tif")
    ...
    def disconnected_areas(self, Q):
        ...
        try:
            ...
            disc_hab_ras_path = os.path.join(self.disc_areas_dir, f"disc_hab_{self.lifestage_code}{fGl.write_Q_str(Q)}.tif")
        ...
        disc_area_path = os.path.join(self.disc_areas_dir, f"disc_area{fGl.write_Q_str(Q)}.shp")
        total_area_path = os.path.join(self.areas_dir, f"area{fGl.write_Q_str(Q)}.shp")
    ...
    def apply_flow_reduction(self):
        ...
        self.logger.info(f"Reducing flow from {self.q_high} to {self.q_low} {self.q_units}")
    ...
    def flow_red_info(self, disc_area):
        ...
        with open(info_path, "w") as info_file:
            ...
            info_file.write(f"\nQ_high: {self.q_high} {self.q_units}")
            info_file.write(f"\nQ_low: {self.q_low} {self.q_units}")
            info_file.write(f"\nApplied species: {self.species}")
            info_file.write(f"\nApplied lifestage: {self.lifestage}")
            info_file.write(f"\nCumulative disconnected area: {disc_area} {self.area_units}")
