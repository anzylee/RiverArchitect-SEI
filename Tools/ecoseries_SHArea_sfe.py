...
case_name = "sfe_316" # sfe_322, sfe_25, sfe_316_RB_v6
Site_ID = 2090
Site_Area = 65700 # contributing area of the corresponding SFE site
case_Area = 7901.08

#timeseries_path = "../00_Flows/" + case_name + "/flow_series_" + case_name + "_sample.xlsx"
timeseries_path = "F:/sfe_hydrology/Eco_performance/SFER/Scenario016.csv"
...
#    f_sfe = pd.read_csv(SFE_LOI_path)

    f3 = pd.read_csv(timeseries_path)
    #f4 = pd.read_excel(timeseries_path)

    Site_OX = np.array(f3).transpose()[3] == Site_ID
    Date = np.array(f3).transpose()[2][Site_OX]
    ...
    Flow_series = Flow_series/Site_Area*case_Area # Area
