            ...
            for ls in fish[species]:
                ...
                for csi in csi_list:
                    ...
                    try:
                        ...
                        dbf_name = self.cache + str(cc) + "table.dbf"

                        arcpy.CheckOutExtension("Spatial")
                        arcpy.sa.TabulateArea(shp_name, "id", shp_name, "id", dbf_name, 0.01, "CLASSES_AS_ROWS")
                        area_dbf = simpledbf.Dbf5(dbf_name)
                        partial_area = area_dbf.to_dataframe()

                        self.logger.info("         ... summing up area ...")
                        area = np.sum(np.array(partial_area['Area']))

                        del area_dbf
                        arcpy.Delete_management(dbf_name)
    ...
    def clear_cache(self, *args):
        ...
        try:
            ...
            for table in table_list:
                try:
                    arcpy.Delete_management(str(table))
                except:
                    pass
            try:
